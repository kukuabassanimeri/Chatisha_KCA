...
def ReportPdf(request):
    ...
    issues = IssueSubmissionModel.objects.filter(
        Q(department = department) | Q(current_owner = user)
    )
    total_all = issues.count()
    total_pending = issues.filter(status = 'pending').count()
    total_resolved = issues.filter(status = 'resolved').count()
    total_forwarded = issues.filter(status = 'forwarded').count()
    
    # PDF RESPOND
    response = HttpResponse(content_type = 'application/pdf')
    response['content-deposition'] = 'attachment; filename = "issue_report_{datetime.now().date().pdf" '
    ...
    p.drawString(50, 770, f'Generated by: { user.get_role_display() }')
    p.drawString(50, 750, f'Date: {datetime.now().strftime('%Y-%m-%d')}')
    
    p.line(50, 640, 550, 640)
    
    # Summary
    p.setFont('Helvetica', 14)
    p.drawString(50, 720, 'Issue Summary')
    p.setFont("Helvetica", 12)
    p.drawString(60, 700, f"Total Issues: {total_all}")
    p.drawString(60, 685, f"Pending: {total_pending}")
    p.drawString(60, 670, f"Resolved: {total_resolved}")
    p.drawString(60, 655, f"Forwarded: {total_forwarded}")
    ...
    p.drawString(50, y, 'Recent Issues')
    p.setFont('Helvetica', 10)
    y -= 20

    for issue in issues:
        p.drawString(50, y, f' {issue.title} - {issue.description} - {issue.status} ')
        y -= 15
        if y < 100:
            p.showPage()
            y = 800
